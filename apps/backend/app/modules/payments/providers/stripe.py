...
Stripe-specific concerns kept here:
  ...
  - `account.updated` webhook → mirrored to User row (handled in
    `app.modules.payouts.service.sync_account_from_event`, not here).
"""
...
from app.modules.hub.snapshot import fork_snapshot_into_agent
...
def _stripe():
    ...
    Exposed at module level (not method) because `app.modules.payouts.service`
